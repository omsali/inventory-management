...
import CustomerSheetCard from '../components/PumpsCard/CustomerSheetCard';

const CustomerSheetPump = () => {
    ...
    const handleDownloadCSV = () => {
        ...
        a.href = 'http://localhost:5000/api/v1/downloadCBS-csv';
        a.download = 'Customer-Sheet-[todays date].csv';
            ...
            <div className='border border-sky-400 shadow-xl shadow-sky-500 rounded-2xl mx-auto my-8 pb-8 bg-sky-300'>
                ...
                {filteredPumps &&
                    filteredPumps.map((pump, index) => {
                        return <CustomerSheetCard pump={pump} index={index} handleFilterPump={handleFilterPump} />
                    })}
            </div>
        </div>
    )
...
export default CustomerSheetPump